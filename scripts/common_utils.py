...
def normalize_hand_robot(robot: str | None) -> str:
    ...
    if r not in ("shadow", "orca", "allegro", "shadowlite"):
        raise ValueError(f"Unknown robot {robot!r}. Use one of: shadow, orca, allegro, shadowlite.")
    return r
...
def register_hand_task_to_hydra(
    ...
    if task_name == "Bounce":
        from roto.tasks.bounce.bounce import BounceAllegroCfg, BounceCfg, BounceOrcaCfg, BounceShadowLiteCfg

        env_cfg_cls = {"shadow": BounceCfg, "orca": BounceOrcaCfg, "allegro": BounceAllegroCfg, "shadowlite": BounceShadowLiteCfg}[r]
    elif task_name == "Baoding":
        from roto.tasks.baoding.baoding import BaodingAllegroCfg, BaodingCfg, BaodingOrcaCfg, BaodingShadowLiteCfg

        env_cfg_cls = {"shadow": BaodingCfg, "orca": BaodingOrcaCfg, "allegro": BaodingAllegroCfg, "shadowlite": BaodingShadowLiteCfg}[r]
    else:
